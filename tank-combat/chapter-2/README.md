...
- **Tap a tank to cycle its state:** *auto-path* (tap a cell and it routes there,
  stopping on arrival; unreachable shows as *no path*) → *manual* (drive it with
  `W`/`S`/`A`/`D` or the on-screen pad, which appears only in this state) →
...
- **The default map** (generated on the host, `tools/gen_map.c` →
  `src/map_data.c`) gives every screen solid borders, an always-open ring one cell
  inside, and a **distinct interior** (blocks, pillars, rooms, dead-ends, a maze,
  an enclosed *island* of unreachable cells, …). Between neighbours there are
  **0, 1, or 2 matching openings** — not every border connects, but a randomised
  spanning tree forces enough that **no screen is an island**. Every opening sits
  on the ring, so a screen's openings are mutually reachable; the ring + tree make
  the world one connected toroidal grid (~80 edge points). A connectivity
  self-check fails the build if any screen is unreachable.
...
both wraps; that a routing tank reaches reachable targets **without its
footprint ever overlapping a wall** and stops on the destination cell; that an
unreachable (sealed) target is reported, not looped on; the **tank state machine**
(cycle, single selection, an UNSELECTED tank keeps pathing, a MANUAL tank ignores
its destination); and that the inherited movement model still holds on the big grid.
...
- `game.wasm` exercised in Node (init, the state cycle, tanks routing to cross-
  screen and wrap destinations and arriving on the exact cell, multiple paths at
  once, wall-toggle table rebuild, camera/slide, and the flow-field JS logic
  matching the C) — identical to native, as they share `sim.c`.
