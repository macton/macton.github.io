...
/* wander outward bias: percent of wander steps that drift away from the nearest nest (0..100) */
EXPORT(wander_bias)      uint32_t wander_bias(void)        { return g_world.wander_bias; }
EXPORT(set_wander_bias)  void     set_wander_bias(uint32_t v) { g_world.wander_bias = (uint8_t)(v > 100 ? 100 : v); }
