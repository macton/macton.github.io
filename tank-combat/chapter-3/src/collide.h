/* collide.h — per-axis grid collision for incremental body moves, plus the
 * world-cell wall query the whole module shares.
 *
 * A body steps less than one cell per tick, and its previous cell was already
 ...
 * leading edge enters, at the rows/columns the body spans on the other axis. The
 * body's half-extent is a `radius` parameter (tanks pass TANK_R, mites MITE_R),
...
/* Body centre of half-extent `radius` moving to (nx,y) by step mx (sign gives
 * the leading edge): does the leading x edge enter a wall, at the rows the body
 * spans at y? */
 ...
 * cell — it indexes the precomputed escape table (see src/escape_table.h). It is
 * size-independent, so both tanks and mites auto-steer off the same table. */
