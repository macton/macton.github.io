...
void agent_move(uint32_t* xy, uint32_t* vxy, uint8_t* hit,
  ...
  for (uint32_t i = 0; i < n; i++) {
    /* hit[i] still holds last tick's collision (turn only read it); if the body
    ...
    if (mx && blocked_x(grid, nx, y, mx, radius)) { mx = 0; h |= 1; } else x = nx;  /* bit0: x blocked */
    int32_t ny = wrap_y(y + my);
    if (my && blocked_y(grid, x, ny, my, radius)) { my = 0; h |= 2; } else y = ny;  /* bit1: y blocked */
