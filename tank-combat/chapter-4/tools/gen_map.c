/* gen_map.c — HOST-ONLY. Emits src/map_data.c (the initial 8x8 world map) to
...
/* a distinct centre for each screen (screen 0 included — no blank screens). With 64
 * screens the 16 interior patterns repeat (s % 16); screen 0 keeps the central block. */
...
/* --- deterministic PRNG + spanning tree over the 8x8 toroidal screen graph -- */
...
#define N_ADJ        (2 * N_SCREENS)   /* each screen contributes its East + South adjacency */
#define MAX_OPENINGS 100               /* cap total openings so edge points stay under N_EDGE_MAX
                                        * (each opening = 2 edge points; uint8 ids cap at 255) */
/* adjacencies: each screen's East and South neighbour (toroidal) */
static int adj_sa[N_ADJ], adj_sb[N_ADJ], adj_type[N_ADJ], adj_open[N_ADJ];  /* type 0=East,1=South */
...
int main(void) {
  ...
  /* randomised spanning tree: shuffle adjacency order, union to connect every screen */
  int order[N_ADJ]; for (int i=0;i<na;i++) order[i]=i;
  for (int i=na-1;i>0;i--){ int j=rng()%(i+1); int t=order[i];order[i]=order[j];order[j]=t; }
  for (int i=0;i<N_SCREENS;i++) par[i]=i;
  int tree[N_ADJ]; for (int i=0;i<na;i++) tree[i]=0;
  for (int i=0;i<na;i++){ int e=order[i]; if (uni(adj_sa[e],adj_sb[e])) tree[e]=1; }
  /* one opening per tree edge guarantees connectivity; spend the rest of the opening budget
   * on extra (non-tree) crossings, in shuffled order, so total openings stay <= MAX_OPENINGS
   * and the edge-point count stays under the uint8 cap. */
  for (int e=0;e<na;e++) adj_open[e] = tree[e] ? 1 : 0;
  int opened = N_SCREENS - 1;                                   /* the tree's openings */
  for (int i=0;i<na && opened<MAX_OPENINGS;i++){ int e=order[i];
    if (!tree[e]) { adj_open[e] = 1; opened++; } }

  /* apply openings (matched cells on both screens, on the ring) */
  for (int e=0;e<na;e++){ int sa=adj_sa[e], sb=adj_sb[e];
  ...
  printf(" * 8x8 world, one 20-bit word per screen-row (bit c == column c, 1 = wall).\n");
